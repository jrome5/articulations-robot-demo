...
namespace Obi
{
    ...
    public class SphereGripperController : MonoBehaviour
    {
        ...
        public GameObject agent_manager;


        private float y_offset = 0.0f;//0.16f;
        ...
        void GrabFirstPoint()
        {
            ...
            var pos = gripper.transform.position;

            //do y last
            if(NearlyEqual(displacement.x, 0f) && NearlyEqual(displacement.z, 0f))
            {
                //gripper.GetComponent<ClothGripper>().SetGrab();

                if(NearlyEqual(displacement.y, 0.0f))
                {
                    //grasp                    
                    if(gripper.GetComponent<ClothGripper>().IsClosed())
                    {
...
//                        agent.SendMessage("StartRequesting");
                    ...
                    {
                        gripper.GetComponent<ClothGripper>().ToggleClosed();
                    }
                }
                else
                {
            ...
            gripper.transform.position = pos;
        }

...
public class GripperObiGrabber : MonoBehaviour
{
    ...
    private void Awake()
    {
        cloth = GameObject.FindGameObjectsWithTag("Cloth")[0];
                        ...
                        if (contact.distance < 0.01f)
                        {
                            ...
                            if (contactCollider == localCollider)
                            {
                                ...
                                if (GrabParticle(solver, particleIndex))
                                    ...
                                    // cloth.SendMessage("GrabbedParticle", particleIndex);
                            }
    ...
    {
        var drive = leftGripper.GetComponent<ArticulationBody>().zDrive;
        bool is_closed = drive.target < -0.25f;
        if(is_closed)
        {
            if(!gripper_closed) //if was previously not closed
            {
                Grab();
                Debug.Log("Grab");
            }

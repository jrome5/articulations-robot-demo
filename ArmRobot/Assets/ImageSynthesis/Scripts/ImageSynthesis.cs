...
public class ImageSynthesis : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        agent.SendMessage("GetCameraObs", GetRGBD());
    }
    ...
    static private void SetUpDepthCamera(Camera cam, Shader shader, ReplacementMode mode, Color clearColor)
    {
        var cb = new CommandBuffer();
        cb.SetGlobalFloat("_OutputMode", (int)mode); // @TODO: CommandBuffer is missing SetGlobalInt() method
        cam.AddCommandBuffer(CameraEvent.BeforeForwardOpaque, cb);
    ...
    public void OnCameraChange()
    {
        ...
        SetUpDepthCamera(capturePasses[3].camera, uberReplacementShader, ReplacementMode.DepthCompressed, Color.white);
    ...
    //steps to getting rgbd camera:
    //1. get the pass variable for rgb and d
    //2. go through function with pass variable
    //3. save to variable
    public (byte[],byte[]) GetRGBD()
    {
        ...
        foreach(var pass in capturePasses)
        {
            ...
            else
            {
                ...
                //no direct slice operation, need to use loops
                if(pass.name == "_img")
                {
                    RGB = bytes;
                }
                else if(pass.name == "_depth")
                {
                    D = bytes;
                }
            }
    ...
    public void SaveMessage(string new_filename)
    {
        //Vector2 gameViewSize = Handles.GetMainGameViewSize();
        //Save(filename, width: (int)gameViewSize.x, height: (int)gameViewSize.y, filepath);
        Save(new_filename, width: 640, height: 480, filepath);
